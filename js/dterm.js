...
(function($) {
   ...
   controller.global = {
      ...
      man: function(e) {
         switch (e) {
            case undefined:
               this.error("man command except at least one argument.");
               break;
               break;
            default:
               this.error("man command not implemented yet. Sorry about that !");
               action.cry(this)
         }
      },
      "..": function() {
   ...
   $.fn.dterm = function(eval, options) {
      var op = $.extend_if_has({}, options, ['greetings', 'prompt', 'onInit',
         'onExit', 'clear',
         'login', 'name', 'exit'
      ]);
      op.enabled = false;
